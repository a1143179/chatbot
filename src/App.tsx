  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf: any) => {
        ...
        vrmRef.current = vrm;
        scene.add(vrm.scene);
        // Position the model to be centered and moved down 150px
        vrm.scene.position.set(0, -0.5, 0); // Center the avatar and move down 150px (1.0 - 1.5 = -0.5)
    ...
    const handleResize = () => {
      if (camera && renderer) {
        ...
        if (vrmRef.current) {
          vrmRef.current.scene.position.set(0, -0.5, 0);
        }
      }
    };
    window.addEventListener('resize', handleResize);
